...
func test_get_card_methods():
	...
	assert_eq(pile.get_child(2),pile.get_top_card(), 
			'get_top_card() returns top card')
	# Likewise, the first card from the bottom is the previous to last.
	assert_eq(pile.get_child(pile.get_child_count() - 2),pile.get_bottom_card(), 
			'get_bottom_card returns() bottom card')
	assert_eq(pile.get_top_card(),pile.get_all_cards()[0],
