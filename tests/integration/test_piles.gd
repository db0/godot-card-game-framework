...
func test_move_to_multiple_container():
	...
	yield(yield_for(0.3), YIELD) # Wait to allow dragging to start
	board.UT_interpolate_mouse_move(cfc.NMAP.deck.position,cards[4].position)
	yield(yield_for(0.6), YIELD) # Wait to allow dragging to start
	...
	yield(yield_for(0.3), YIELD) # Wait to allow dragging to start
	board.UT_interpolate_mouse_move(cfc.NMAP.discard.position,cards[1].position)
	yield(yield_for(0.6), YIELD) # Wait to allow dragging to start
	...
	yield(yield_for(0.3), YIELD) # Wait to allow dragging to start
	board.UT_interpolate_mouse_move(cfc.NMAP.deck.position,cards[0].position)
	yield(yield_for(0.6), YIELD) # Wait to allow dragging to start
