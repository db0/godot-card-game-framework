...
func test_single_card_draw():
	...
	yield(yield_to(card0.get_node('Tween'), "tween_all_completed", 1), YIELD)
	assert_almost_eq(415.0,card0._recalculatePosition().x,2.0,
			"Card position x is recalculated correctly")
	assert_almost_eq(120.0,card0._recalculatePosition().y,2.0,
			"Card position y is recalculated correctly")
	assert_almost_eq(hand.to_global(card0._recalculatePosition()),
			card0.global_position,Vector2(2,2),
			"Card placed in correct global position")
	assert_almost_eq(card0._recalculatePosition(),card0.position,Vector2(2,2),
...
func test_draw_multiple_cards_slow():
	...
	assert_almost_eq(card1._recalculatePosition(),card1.position,Vector2(2,2),
	...
	yield(yield_to(card2.get_node('Tween'), "tween_all_completed", 1), YIELD)
	assert_almost_eq(250.0,card0._recalculatePosition().x,2.0,
			"Index 0 card position x is recalculated correctly")
	assert_almost_eq(120.0,card0._recalculatePosition().y,2.0,
			"Index 0 card position y is recalculated correctly")
	assert_almost_eq(415.0,card1._recalculatePosition().x,2.0,
			"Index 1 card position x is recalculated correctly")
	assert_almost_eq(120.0,card1._recalculatePosition().y,2.0,
			"Index 1 card position y is recalculated correctly")
	assert_almost_eq(580.0,card2._recalculatePosition().x,2.0,
			"Index 2 card position x is recalculated correctly")
	assert_almost_eq(120.0,card2._recalculatePosition().y,2.0,
			"Index 2 card position y is recalculated correctly")
	assert_almost_eq(card0._recalculatePosition(),card0.position,Vector2(2,2),
			"Card at index 0 placed in correct position")
	assert_almost_eq(card1._recalculatePosition(),card1.position,Vector2(2,2),
			"Card at index 1 placed in correct position")
	assert_almost_eq(card2._recalculatePosition(),card2.position,Vector2(2,2),
...
func test_draw_multiple_cards_fast():
	...
	assert_almost_eq(card2._recalculatePosition(),card2.position,Vector2(2,2),
			"Card at index 2 placed in correct position")
	assert_almost_eq(card3._recalculatePosition(),card3.position,Vector2(2,2),
			"Card at index 3 placed in correct position")
	assert_almost_eq(card4._recalculatePosition(),card4.position,Vector2(2,2),
			"Card at index 4 placed in correct position")
	assert_almost_eq(card5._recalculatePosition(),card5.position,Vector2(2,2),
