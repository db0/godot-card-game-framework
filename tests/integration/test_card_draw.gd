...
func test_single_card_draw():
	...
	if cfc.hand_use_oval_shape:
		...
		assert_almost_eq(415.0,card0.recalculate_position(card0).x,2.0,
				"Card position x is recalculated correctly")
		assert_almost_eq(120.0,card0.recalculate_position(card0).y,2.0,
				"Card position y is recalculated correctly")
	assert_almost_eq(hand.to_global(card0.recalculate_position(card0)),
			card0.global_position,Vector2(2,2),
			"Card placed in correct global position")
	assert_almost_eq(card0.recalculate_position(card0),card0.position,Vector2(2,2),
...
func test_draw_multiple_cards_slow():
	...
	assert_almost_eq(card1.recalculate_position(card1),card1.position,Vector2(2,2),
	...
	if cfc.hand_use_oval_shape:
		...
		assert_almost_eq(250.0,card0.recalculate_position(card0).x,2.0,
				"Index 0 card position x is recalculated correctly")
		assert_almost_eq(120.0,card0.recalculate_position(card0).y,2.0,
				"Index 0 card position y is recalculated correctly")
		assert_almost_eq(415.0,card1.recalculate_position(card1).x,2.0,
				"Index 1 card position x is recalculated correctly")
		assert_almost_eq(120.0,card1.recalculate_position(card1).y,2.0,
				"Index 1 card position y is recalculated correctly")
		assert_almost_eq(580.0,card2.recalculate_position(card2).x,2.0,
				"Index 2 card position x is recalculated correctly")
		assert_almost_eq(120.0,card2.recalculate_position(card2).y,2.0,
				"Index 2 card position y is recalculated correctly")
	assert_almost_eq(card0.recalculate_position(card0),card0.position,Vector2(2,2),
			"Card at index 0 placed in correct position")
	assert_almost_eq(card1.recalculate_position(card1),card1.position,Vector2(2,2),
			"Card at index 1 placed in correct position")
	assert_almost_eq(card2.recalculate_position(card2),card2.position,Vector2(2,2),
...
func test_draw_multiple_cards_fast():
	...
	assert_almost_eq(card2.recalculate_position(card2),card2.position,Vector2(2,2),
			"Card at index 2 placed in correct position")
	assert_almost_eq(card3.recalculate_position(card3),card3.position,Vector2(2,2),
			"Card at index 3 placed in correct position")
	assert_almost_eq(card4.recalculate_position(card4),card4.position,Vector2(2,2),
			"Card at index 4 placed in correct position")
	assert_almost_eq(card5.recalculate_position(card5),card5.position,Vector2(2,2),
