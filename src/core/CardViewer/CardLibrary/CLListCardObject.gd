...
# Extends the name and type to present all card properties
class_name CLListCardObject
extends CVListCardObject

var default_property_width := 30
var property_width_exceptions: Dictionary
...
func setup(_card_name: String) -> void:
	...
	if property_width_exceptions.has(CardConfig.SCENE_PROPERTY):
		_card_type.custom_minimum_size.x = property_width_exceptions[CardConfig.SCENE_PROPERTY]
	else:
		_card_type.custom_minimum_size.x = default_property_width
