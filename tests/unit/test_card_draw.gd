...
func test_single_card_draw():
	...
	#yield(yield_to(card.get_node('Tween'), "tween_all_completed", 2), YIELD)
	yield(yield_for(1), YIELD)
	assert_eq(Vector2(565,600),card0.rect_global_position, "Check card placed in correct global position")
